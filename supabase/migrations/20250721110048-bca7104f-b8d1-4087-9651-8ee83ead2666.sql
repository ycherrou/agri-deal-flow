...
BEGIN
  ...
  -- Obtenir la taille du contrat pour ce produit (cast explicite)
  ...
  IF vente_record.type_deal = 'prime' THEN
    -- Conserver le type prime avec la prime originale et le prix bid comme référence
    INSERT INTO ventes (
      ...
      vente_record.prime_vente, bid_record.volume_bid, CURRENT_DATE, 
      vente_record.prix_reference
    ) RETURNING id INTO new_vente_id;
  ELSE
    -- Pour les deals flat, utiliser le prix bid comme nouveau prix flat
    INSERT INTO ventes (
  ...
  -- Notification pour l'acheteur (offre acceptée)
  ...
  -- Notification pour le vendeur (transaction complétée)
  PERFORM net.http_post(
    url := 'https://ghakstxearfsffrbkuxs.supabase.co/functions/v1/send-whatsapp-notification',
    headers := '{"Content-Type": "application/json", "Authorization": "Bearer ' || 
      current_setting('app.settings.service_role_key', true) || '"}'::jsonb,
    body := json_build_object(
      ...
      'variables', json_build_object(
        ...
        'volume', bid_record.volume_bid,
        'produit', navire_record.produit,
        'gain', ROUND(gain_vendeur * bid_record.volume_bid, 2)
      )
    )::jsonb
  );

  -- Notification pour l'acheteur (transaction complétée)
  PERFORM net.http_post(
    url := 'https://ghakstxearfsffrbkuxs.supabase.co/functions/v1/send-whatsapp-notification',
    headers := '{"Content-Type": "application/json", "Authorization": "Bearer ' || 
      current_setting('app.settings.service_role_key', true) || '"}'::jsonb,
