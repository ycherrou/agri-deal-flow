-- Mettre à jour la fonction calculate_pru_vente pour inclure le fret pour les navires FOB
CREATE OR REPLACE FUNCTION public.calculate_pru_vente(vente_id_param uuid)
 RETURNS numeric
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
AS $function$
DECLARE
  vente_record record;
  navire_record record;
  facteur_conversion numeric;
  prix_futures_moyen numeric;
  pru numeric;
  prix_achat_base numeric;
BEGIN
  -- Get sale record
  SELECT * INTO vente_record 
  FROM ventes 
  WHERE id = vente_id_param;
  
  IF NOT FOUND THEN
    RETURN 0;
  END IF;
  
  -- Get vessel record for product type
  SELECT * INTO navire_record 
  FROM navires 
  WHERE id = vente_record.navire_id;
  
  -- Determine conversion factor based on product type (cts/bu -> $/tonne)
  CASE navire_record.produit
    WHEN 'mais' THEN facteur_conversion := 0.3937;
    WHEN 'tourteau_soja' THEN facteur_conversion := 0.9072;
    ELSE facteur_conversion := 1; -- Already in $/tonne
  END CASE;
  
  -- Calculate based on deal type and vessel type
  CASE vente_record.type_deal
    WHEN 'flat' THEN 
      -- For flat deals, PRU equals the flat price (already in $/tonne)
      prix_achat_base := COALESCE(vente_record.prix_flat, navire_record.prix_achat_flat, 0);
      
      -- Add freight if vessel is FOB
      IF navire_record.terme_commercial = 'FOB' THEN
        pru := prix_achat_base + COALESCE(navire_record.taux_fret, 0);
      ELSE
        pru := prix_achat_base;
      END IF;
      
    WHEN 'prime' THEN 
      -- For prime deals: (prime_vente - prime_achat + average_futures_price) * conversion_factor + freight_if_FOB
      SELECT AVG(c.prix_futures) INTO prix_futures_moyen
      FROM couvertures c 
      WHERE c.vente_id = vente_record.id;
      
      -- Calculate prime difference in $/tonne
      prix_achat_base := (COALESCE(vente_record.prime_vente, 0) - COALESCE(navire_record.prime_achat, 0) + COALESCE(prix_futures_moyen, 0)) * facteur_conversion;
      
      -- Add freight if vessel is FOB
      IF navire_record.terme_commercial = 'FOB' THEN
        pru := prix_achat_base + COALESCE(navire_record.taux_fret, 0);
      ELSE
        pru := prix_achat_base;
      END IF;
      
    ELSE 
      pru := 0;
  END CASE;
  
  RETURN pru;
END;
$function$