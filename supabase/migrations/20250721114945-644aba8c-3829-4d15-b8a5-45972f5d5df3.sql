-- Corriger la fonction accept_bid_and_create_transaction pour bien gérer les primes
CREATE OR REPLACE FUNCTION public.accept_bid_and_create_transaction_with_notifications(
  bid_id_param uuid,
...
DECLARE
  ...
  prix_achat_original_prime numeric; -- Prime d'achat originale
  prix_vente_final_prime numeric;    -- Prime de vente finale
  gain_vendeur_prime numeric;        -- Gain en prime (cts/bu)
  gain_vendeur_total numeric;        -- Gain total (USD)
  transaction_id uuid;
  new_vente_id uuid;
...
BEGIN
  ...
  SELECT * INTO buyer_client FROM clients WHERE id = bid_record.client_id;
  SELECT * INTO seller_client FROM clients WHERE id = seller_client_id;
  ...
  -- Pour les deals prime : calculer les gains en prime (cts/bu)
  IF vente_record.type_deal = 'prime' THEN
    prix_achat_original_prime := COALESCE(vente_record.prime_vente, 0);
    prix_vente_final_prime := bid_record.prix_bid; -- Le prix_bid est la prime demandée
  ...
  contract_size := get_contract_size(navire_record.produit::text);
  ...
  IF vente_record.type_deal = 'prime' THEN
    INSERT INTO ventes (
      navire_id, client_id, type_deal, prime_vente, volume, date_deal, prix_reference
    ) VALUES (
